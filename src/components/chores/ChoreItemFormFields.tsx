...
import { type ChoreItemFormState } from "@/lib/choreItemForm";
  ...
  return (
    <div className="space-y-4">
      ...
      <ChoreLocationPicker
        ...
        onSelectZone={(zoneId) => {
          const zone = homeZones.find((z) => z.id === zoneId);
          onChange({
            ...form,
            zoneId,
            mapShape: "zone",
            room: zone?.room ?? form.room,
            mapPins: [],
            mapLine: [],
            mapArea: [],
          });
        ...
        <>
          <div className="space-y-1.5">
            ...
            <Select
              ...
              onValueChange={(v) =>
                setForm({ recurrence: v as TaskRecurrence })
              }
            >
              <SelectTrigger>
                <SelectValue />
          ...
          <SelectContent>
            {Array.from(
              ...
              <SelectItem key={level} value={String(level)}>
                {level}
                {" — "}
                {IMPORTANCE_LABELS[level as TaskImportance] ??
                  `Poziom ${level}`}
              </SelectItem>
            ))}
          </SelectContent>
      ...
      {form.mapShape === "zone" && form.zoneId && (
        <p className="text-xs text-muted-foreground">
          Pozycja obejmuje całą wybraną strefę (bez dodatkowej geometrii).
        </p>
      )}
    </div>
  );

  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Coś poszło nie tak. Spróbuj odświeżyć (Ctrl+Shift+R) albo wyczyść dane
          strony w ustawieniach przeglądarki — stary cache często blokuje
          wczytanie po aktualizacji.
        </p>
        {error?.message ? (
  ...
  return (
    <html lang="en">
      <head>
        <HeadContent />
        <script
          dangerouslySetInnerHTML={{ __html: CHUNK_RELOAD_GUARD_SCRIPT }}
        />
      </head>
      <body>
        {children}
